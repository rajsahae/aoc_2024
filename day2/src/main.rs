...
#[derive(Debug)]
struct ReportPair(usize, usize);

impl ReportPair {
    fn diff(&self) -> usize {
        self.0.max(self.1) - self.0.min(self.1)
    }
}

impl Report {
    fn new(data: Vec<usize>, dampened: bool) -> Self {
        Self {
            data,
            mode: Mode::Unknown,
            dampened,
    ...
    /// Returns a new subreport with specified layer removed
    fn subreport(&self, idx: usize) -> Self {
        let mut new_data = self.data.clone();
        new_data.remove(idx);
        Self::new(new_data, true)
    }

    /// Returns true if all report layers are safe
    fn check_safety(&mut self) -> bool {
        let indices: Vec<usize> = (0..self.data.len()).collect::<Vec<usize>>();
        for win in indices.windows(2) {
            let pair = ReportPair(self.data[win[0]], self.data[win[1]]);

            if !self.check_mode(&pair) || !self.check_diff(&pair) {
                if self.dampened {
                    return false;
                } else {
                    // TODO: this could be more performant if we created a subreport
                    // from the current pos onwards without the bad index but
                    // it seems to run fine without it so not gaing to bother.

                    let mut damp_left = self.subreport(win[0]);
                    let mut damp_right = self.subreport(win[1]);

                    let check_prev = if win[0] > 0 {
                        let mut damp_prev = self.subreport(win[0] - 1);
                        damp_prev.check_safety()
                    } else {
                        false
                    };

                    return check_prev || damp_left.check_safety() || damp_right.check_safety();
                }
            }
        }
    ...
    fn check_mode(&mut self, pair: &ReportPair) -> bool {
        let left = pair.0;
        let right = pair.1;
    ...
    fn check_diff(&self, pair: &ReportPair) -> bool {
        0 < pair.diff() && pair.diff() <= 3
    }
}

impl From<&[usize]> for Report {
    fn from(val: &[usize]) -> Self {
        Self::new(val.to_vec(), false)
    }
}
