...
fn main() {
    println!("{}", MulScan::scan(PUZZLE_INPUT));
}
...
const MUL_PAT: &str = r"mul\((\d{1,3}),(\d{1,3})\)";
...
impl MulScan {
    fn scan(input: &str) -> usize {
        ...
        Self::split(input)
            .map(|segment| Self::mul_scan(segment))
            .sum()
    }

    fn split(input: &str) -> impl Iterator<Item = &str> {
        let dont_rxp = Regex::new(r"don't\(\)").unwrap();
        let do_rxp = Regex::new(r"do\(\)").unwrap();
        ...
        loop {
            let current_segment = input.get(start..).unwrap();

            if record {
                if let Some(md) = dont_rxp.find(current_segment) {
                    let valid_segment = current_segment.get(..md.start()).unwrap();
                    segments.push(valid_segment);
                    start += md.end();
                    record = false;
                } else {
                ...
                if let Some(md) = do_rxp.find(current_segment) {
                    start += md.end();
                    record = true;
                } else {
                    break;
    ...
    fn mul_scan(input: &str) -> usize {
        let re = Regex::new(MUL_PAT).unwrap();
        let scans: Vec<MulScan> = re
            .find_iter(input)
            .map(|m| MulScan::parse(m.as_str()))
            .collect();
        scans.iter().map(|ms| ms.compute()).sum()
    }

    fn parse<'a>(input: &'a str) -> Self {
        let re = Regex::new(MUL_PAT).unwrap();
        let caps = re.captures(input).unwrap();
        let lhs = caps.get(1).unwrap().as_str().parse::<usize>().unwrap();
        let rhs = caps.get(2).unwrap().as_str().parse::<usize>().unwrap();
        Self { lhs, rhs }
    }
